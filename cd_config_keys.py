...
#   cmd_id      = 524391
    ...
    while True:
        ...
        itms    = (zip([_('Command')+stat_c, _('Hotkey-1')+stat_k1, _('Hotkey-2')+stat_k2, _('Snips')+stat_s], map(str, COL_WS))
                  ,    [ (nm,                 k1,             k2,          ', '.join(sns)) 
        ...
        cnts    =([]
                ...
                +([] if not sndt else []
                 +[dict(           tp='lb'  ,tid='orsn' ,l=lfsn ,w=50   ,cap=_('In &Snips:')            ,hint=scnd_h)] # &s
                ...
                +([] if not apx.get_opt('config_keys_with_open', False) else []
                 ...
                 +[dict(cid='trpt',tp='bt'  ,t=DLG_H-30 ,l=130  ,w=150  ,cap=_('Report to new &Tab')    ,hint=trpt_h)] # &t
        ...
        btn, vals, fid, chds = dlg_wrapper(_('Config Hotkeys'), DLG_W, DLG_H, cnts, vals, focus_cid=focused)
...
#                                                               ,on_click_header=lambda idd, idc, data:print('lwks',data) )) #     grid
 ...
 ,('trpt',dict(tp='bt'  ,t=M.DLG_H-30   ,l=130      ,w=150  ,cap=_('Report to new &Tab'),hint=M.trpt_h              ,call=m.do_rprt )) # &t
 ,('add1',dict(tp='bt'  ,t=M.DLG_H-60   ,l=M.lfk1   ,w=150  ,cap=_('Set/Add Hotkey-&1') ,hint=M.addk_h              ,call=m.do_work )) # &1
 ,('del1',dict(tp='bt'  ,t=M.DLG_H-30   ,l=M.lfk1   ,w=150  ,cap=_('Remove Hotkey-1 &!')                            ,call=m.do_work )) # &!
 ,('add2',dict(tp='bt'  ,t=M.DLG_H-60   ,l=M.lfk2   ,w=150  ,cap=_('Set/Add Hotkey-&2') ,hint=M.addk_h              ,call=m.do_work )) # &2
 ,('del2',dict(tp='bt'  ,t=M.DLG_H-30   ,l=M.lfk2   ,w=150  ,cap=_('Remove Hotkey-2 &@')                            ,call=m.do_work )) # &@
 ,('asnp',dict(tp='bt'  ,t=M.DLG_H-60   ,l=M.lfsn   ,w=150  ,cap=_('Set/A&dd Snip')                     ,vis=sndt_b ,call=m.do_work )) # &d
 ,('rsnp',dict(tp='bt'  ,t=M.DLG_H-30   ,l=M.lfsn   ,w=150  ,cap=_('R&emove Snip(s)')                   ,vis=sndt_b ,call=m.do_work )) # &e
 ,('help',dict(tp='bt'  ,t=M.DLG_H-60   ,l=M.lrpt   ,w=100  ,cap=_('Hel&p')                                         ,call=m.do_shlp )) # &p 
                ]
        return cnts
       #def get_cnts
    ...
    def do_shlp(self, aid, ag):
        m,M         = self,CfgKeysDlg
        DW, DH      = M.DLG_W-2*GAP, M.DLG_H-2*GAP
        dlg_wrapper(_('Help for "Config Hotkeys"'), M.DLG_W, M.DLG_H,
             [dict(cid='htxt',tp='me'    ,t=GAP  ,h=DH-28,l=GAP          ,w=DW   ,props='1,0,1'  ) #  ro,mono,border
             ,dict(cid='-'   ,tp='bt'    ,t=GAP+DH-23    ,l=GAP+DW-80    ,w=80   ,cap=_('&Close'))
             ], dict(htxt=
